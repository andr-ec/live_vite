...
test.describe("useEventReplyReact E2E Tests", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/react-event-reply-test")
    await syncLV(page)
  })
  ...
  test("multiple different hooks work independently", async ({ page }) => {
  ...
  test("component state persists across LiveView updates", async ({ page }) => {
    ...
    // Trigger another server update that doesn't affect this hook
    await page.locator("[data-pw-fetch-user-1]").click()
    await syncLV(page)
