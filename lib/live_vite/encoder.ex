defprotocol LiveVite.Encoder do
  @moduledoc """
  Protocol for encoding values to JSON for LiveVite.
  ...
  * `:except` - encodes all struct fields except specified keys.
  ...
  If we were to call `@derive LiveVite.Encoder` just before `defstruct`, an
  ...
  If we called `@derive {LiveVite.Encoder, only: [:name, :email]}`, only the
  ...
  If we called `@derive {LiveVite.Encoder, except: [:password]}`, all fields
  ...
  Protocol.derive(LiveVite.Encoder, User, only: [...])
...
defimpl LiveVite.Encoder, for: Integer do
  def encode(value, _opts), do: value
end

defimpl LiveVite.Encoder, for: Float do
  def encode(value, _opts), do: value
end

defimpl LiveVite.Encoder, for: BitString do
  def encode(value, _opts), do: value
end

defimpl LiveVite.Encoder, for: Atom do
  def encode(atom, _opts), do: atom
end

# Complex types that need recursive encoding
defimpl LiveVite.Encoder, for: List do
  def encode(list, opts) do
    Enum.map(list, &LiveVite.Encoder.encode(&1, opts))
  end
end

defimpl LiveVite.Encoder, for: Map do
  def encode(map, opts) do
    Map.new(map, fn {key, value} ->
      {key, LiveVite.Encoder.encode(value, opts)}
    end)
  end
end

# Date and time types - Jason encoder handles them
defimpl LiveVite.Encoder, for: [Date, Time, NaiveDateTime, DateTime] do
...
defimpl LiveVite.Encoder, for: Phoenix.HTML.Form do
  def encode(%Phoenix.HTML.Form{} = form, opts) do
    LiveVite.Encoder.encode(
      %{
        name: form.name,
  ...
  if Code.ensure_loaded?(Ecto) do
    defp maybe_enhance_error(%{value: %Ecto.Association.NotLoaded{}} = error) do
      Map.update!(error, :description, fn description ->
        ...
        addition = """
        To prevent this error from happening in forms, you can explicitly encode form using LiveVite.Encoder.encode(form, nilify_not_loaded: true) option.
        """
  ...
  if Code.ensure_loaded?(Ecto) do
    ...
    def encode_form_values(%{impl: Phoenix.HTML.FormData.Ecto.Changeset, source: source}, opts) do
      source |> collect_changeset_values(opts) |> LiveVite.Encoder.encode(opts)
    end
  end
  ...
  def encode_form_values(form, opts) do
    ...
    LiveVite.Encoder.encode(base_values, opts)
  end
  ...
  defp translate_error({msg, opts}) do
    backend = Application.get_env(:live_vite, :gettext_backend, nil)
    count = opts[:count]
...
defimpl LiveVite.Encoder, for: Any do
  defmacro __deriving__(module, struct, opts) do
    fields = fields_to_encode(struct, opts)

    quote do
      defimpl LiveVite.Encoder, for: unquote(module) do
        def encode(struct, opts) do
          struct
  ...
  def encode(%{__struct__: module} = struct, _opts) do
    raise Protocol.UndefinedError,
      ...
      value: struct,
      description: """
      LiveVite.Encoder protocol must always be explicitly implemented.

      It's used to encode structs to JSON for LiveVite. It's very similar to Jason.Encoder,
      but it's converting structs to maps so LiveVite can diff them correctly.

      If you own the struct, you can derive the implementation specifying \
      which fields should be encoded:

          defmodule #{inspect(module)} do
            @derive {LiveVite.Encoder, only: [...]}
            defstruct ...
          end
      ...
      you may use Protocol.derive/3 placed outside of any module:
          ...
          Protocol.derive(LiveVite.Encoder, #{inspect(module)}, only: [...])
          Protocol.derive(LiveVite.Encoder, #{inspect(module)})

      Nothing prevents you from defining your own implementation for the struct:

      defimpl LiveVite.Encoder, for: #{inspect(module)} do
        def encode(struct, opts) do
          struct
          |> Map.take([:first, :second])
          |> LiveVite.Encoder.encode(opts)
...
defimpl LiveVite.Encoder, for: Phoenix.LiveView.AsyncResult do
...
# Explicit implementation of LiveVite.Encoder for UploadConfig
defimpl LiveVite.Encoder, for: Phoenix.LiveView.UploadConfig do
  def encode(%Phoenix.LiveView.UploadConfig{} = struct, opts) do
    errors =
      Enum.map(struct.errors, fn {key, value} ->
        %{ref: key, error: LiveVite.Encoder.encode(value, opts)}
      end)

    entries =
      Enum.map(struct.entries, fn entry ->
        encoded = LiveVite.Encoder.encode(entry, opts)
...
# Explicit implementation of LiveVite.Encoder for UploadEntry
defimpl LiveVite.Encoder, for: Phoenix.LiveView.UploadEntry do
